...
(function (global, factory) {
  ...
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, global["dep-tree"] = factory(global.node_fs));
  ...
  class DepTreeHelp extends ParseHelp {
    ...
    async getTree(loc) {
      ...
      const matchs = await ph.parse(loc); // localdep-only-exsits
      ...
      this.deptree = deptree; // return deptree;
    }
    ...
    leaves() {
      ...
      alldeps.forEach(name => {
        const matchs = deptree[name]; // has no dep
    ...
    orphans() {
      ...
      alldeps.forEach(name => {
        const matchs = deptree[name];
        if (!matchs || matchs.length === 0) return; // no one is depending on
