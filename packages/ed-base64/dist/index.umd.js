...
(function (global, factory) {
    ...
    (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global["ed-base64"] = {}));
    ...
    function encode$1(text, map) {
      ...
      while (i < text.length) {
        ...
        let enc2;
        let enc3;
        let enc4; // get char 1,2,3

        char1 = text.charCodeAt(i++);
        char2 = text.charCodeAt(i++);
        char3 = text.charCodeAt(i++); // get enc 1,2,3,4

        enc1 = char1 >> 2; // 取第 1 字节的前 6 位

        if (isNaN(char2)) {
          // 只有 1 字节的时候
          enc2 = (char1 & 3) << 4 | 0 >> 4; // 第65个字符用来代替补位的 = 号
