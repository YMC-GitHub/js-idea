/**
  ...
  * @license MIT
  */
import { resolve as resolve$1, sep, extname } from 'node:path';
...
function resloveAlias(data) {
    ...
    const root = resolve$1(process.cwd(), '.');
    ...
    Object.keys(alias).forEach(k => {
        ...
        // ./ -> abs
        alias[k] = resolve$1(root, alias[k]);
    });
    return alias
}
/**
...
const supportsNodePrefix =
    compare(process.version, '14.13.1') >= 0 || compare(process.version, '12.20.0') >= 0;
...
async function tryDirectory(url, context, next) {
    ...
    try {
        ...
        const { message } = error;
        error.message = error.message.replace(`${appendIndex.replace('/', sep)}'`, "'");
