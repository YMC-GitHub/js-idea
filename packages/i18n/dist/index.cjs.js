...
function truthly(s) {
  return s !== false; // return !isDefine(s) || isEmpty(s) || s; eq ? s!=false
}
/**
 * @description
 * ```
...
class NameAlias {
  ...
  getName(alias) {
    const {
      aliasMap
    } = this; // [{name:[alias1,alias2]}]

    const res = [];
    const list = Object.keys(aliasMap); // order-diff when foreach,map,filter ...
    // list.forEach((name) => {
...
function iniLangVal(data, language, val, express) {
  iniObjValByKey(data, language, val); // iniObjValByKey(data[language], express, val);
}
/**
 * @description
 * ```
...
class I18n extends NameAlias {
  ...
  lang(lang) {
    let context; // context = this.data; //bind lang to data.current
  ...
  translate(express, def) {
    let txt; // let language = this.lang();

    const {
      defaultLang
    } = this;
    txt = this.getName(express); // get

    let res; // res = this.data[language][express];//not rewrite here,please use it.set method
  ...
  set(express, def) {
    ...
    const language = this.lang(); // // get-one
    ...
    if (isArray(txt)) {
      txt.forEach(item => {
        if (isArray(item)) {
          const [key, val] = item;
          this.set(key, val); // this.data[language][item[0]]=item[1]
